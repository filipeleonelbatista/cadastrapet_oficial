...
export function AuthContextProvider(props) {
  ...
  function removeKeyLocalStorage(key) {
    const hasLocalStorageData = !(AsyncStorage.getItem(key) === null);
  ...
  function handleForgotUser(email) {
    ...
    sendPasswordResetEmail(authentication, email)
      ...
      .catch((err) => {
        console.log(err);
        Alert.alert("Email não encontrado", AuthErrorHandler[[err.code]]);
      ...
      .then(async (re) => {
        ...
        try {
          ...
          sendDiscordNotification(
            `Novo cadastro realizado pelo appNovo cadastro realizado pelo web app
            **ID:** ${newUser.uid}
            **Nome:** ${newUser.name}
            **Email:** ${newUser.email}
