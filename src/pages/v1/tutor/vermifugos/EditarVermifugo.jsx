...
export default function EditarVermifugo() {
  const navigate = useNavigate();
  const { setIsLoading } = useLoading();
  const { addToast } = useToast();
  const { props, functions } = useAuth();
  const { selectedPet, selectedMedication } = props;
  const {
  ...
  const [selectedAttachment, setSelectedAttachment] = useState([...selectedMedication?.medication_receipt])
  ...
  const formik = useFormik({
    initialValues: {
      ...
      medication_application_date: dayjs(new Date(selectedMedication?.medication_application_date)).format("YYYY-MM-DD"),
      selectedImage: '',
    },
  ...
  const handleSubmitForm = async (formValues) => {
    ...
    if (formValues.selectedImage !== '') {
      ...
      uploadURLImage = selectedMedication?.medication_receipt;
    }
    ...
    if (await updateMedicationByID(selectedMedication.uid, data, selectedPet)) {
      addToast({
        message: "Aplicação do medicamento atualizada com sucesso!"
      })
            ...
            <Box component="form" onSubmit={formik.handleSubmit} sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Button onClick={() => {
                if (window.confirm("Deseja descartar as alterações?")) {
                  formik.resetForm();
                  navigate("/tutor/historico-vermifugos")
                }
