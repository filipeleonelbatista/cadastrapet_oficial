...
export default function AdicionarHistorico() {
  ...
  const formik = useFormik({
    initialValues: {
      consulta: '',
      dt_consulta: dayjs(Date.now()).format("YYYY-MM-DD"),
      anotacoes: '',
      selectedImage: '',
    },
  ...
  const handleSubmitForm = async (formValues) => {
    ...
    if (await updateMedicalHistoryByID(medicalHistoryID, data, selectedPet)) {
      addToast({
        message: "Histórico médico cadastrado com sucesso!"
      })
  ...
  return (
    <>
      ...
      <DrawerComponent title="Adicionar Histórico Médico">
        <Box>
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs}>
          <ContainerComponent>
            <Box component="form" onSubmit={formik.handleSubmit} sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Button type='submit' variant="contained" color="primary">Adicionar histórico</Button>
            </Box>
          </ContainerComponent>
